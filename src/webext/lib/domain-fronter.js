...
(() => {
    ...
    DOMAIN_FRONTS.set('oauth.accounts.firefox.com', 'oauth.accounts.firefox.com.');
    DOMAIN_FRONTS.set('accounts.firefox.com', 'accounts.firefox.com.');

    const REQUEST_ID_HEADER = 'foxified-random-id-to-map-xhr-to-domain-fronter';
    let thisTabId = -1;
    ...
    class DomainFronter {
        ...
        _onBeforeSendHeaders = ({originUrl, requestId, requestHeaders, url}) => {
            if (new URL(originUrl).origin !== location.origin) {
                ...
                console.log('_onBeforeSendHeaders return |', 'originUrl:', originUrl, 'requestId:', requestId, 'requestHeaders:', requestHeaders, 'url:', url);
                return;
            }
            console.log('_onBeforeSendHeaders continue |', 'originUrl:', originUrl, 'requestId:', requestId, 'requestHeaders:', requestHeaders, 'url:', url);
            ...
            const wasOrigRequestWithCredentails = !!this.cookies;
            if (wasOrigRequestWithCredentails) {
                const preExistingCookieHeader = requestHeaders.find(({ name }) => /^cookie$/i.test(name));
                const cookieHeader = preExistingCookieHeader || { name: 'Cookie', value: '' };

                if (cookieHeader.value) {
                    cookieHeader.value += ';';
                }

                cookieHeader.value += this.cookies.map(cookie => `${cookie.name}=${cookie.value}`).join(';');

                if (!preExistingCookieHeader) {
        ...
        _onHeadersReceived({requestId, responseHeaders, url, statusCode}) {
            if (this.requestId !== requestId) {
                console.log('_onHeadersReceived return |', 'requestId:', requestId, 'responseHeaders:', responseHeaders, 'url:', url, 'statusCode:', statusCode);
                return;
            }
            console.log('_onHeadersReceived continue |', 'requestId:', requestId, 'responseHeaders:', responseHeaders, 'url:', url, 'statusCode:', statusCode);
            ...
            if (header) {
                console.log(`REDIRECT!!!! new url: "${url}"`);
    ...
    XMLHttpRequest.prototype.open = function(...args) {
        ...
        if (domainFronter.modifyBeforeRequest(url)) {
            if (this.withCredentials) {
                cookiesPromise = browser.cookies.getAll({ domain: domainFronter.originalDomain });
                cookiesPromise.then(cookies => {
                    domainFronter.cookies = cookies;
                });
            }
            args[1] = url.href;
        }
        ...
        this.send = function(data) {
            Promise.all([cookiesPromise, registeredPromise]).then(() => {
                this.send(data);
            });
        };
    };
